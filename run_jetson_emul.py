...
from analyzers_jetson import analyzer
...
settings_list = [setting2,setting3,setting6]
...
for eval_setting in settings_list:
    ...
    json_dir = f'./results/json_jetson_emul/{scheme}'
    ...
    for key, value in result_dict.items():
        ...
        plt_dict['tracking_time'].append(10*value['tracking_time'])
        plt_dict['profile_time'].append(60*tot_profile_time/len(result_dict))

    inference_latency = np.mean(jetson_inference_dict[res][det_model_name])/1000 # millisecond
    eps = 1e-7
    plt_dict['x'] = [short_video_length/(d*inference_latency + t_t + t_p) for d,t_t,t_p in # microseconds 
